...
async def register(data:RegisterRequest, session:AsyncSession, admin_setup_token:str|None=None)->User:
    ...
    if data.role == UserRole.admin:
        ...
        if not expected or admin_setup_token != expected:
            raise AppError('admin_setup_required','Admin registration requires X-Admin-Setup-Token',403)
        role = UserRole.admin
