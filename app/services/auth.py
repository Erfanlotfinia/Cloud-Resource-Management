...
async def register(data:RegisterRequest, session:AsyncSession)->User:
    ...
    user=User(email=data.email, hashed_password=hash_password(data.password), role=UserRole.user); session.add(user); await session.commit(); await session.refresh(user); return user
