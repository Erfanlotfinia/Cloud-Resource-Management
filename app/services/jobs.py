...
async def rate_limit(user_id: int) -> None:
    ...
    try:
        ...
        log.warning(
            "rate_limit_bypassed_due_to_redis_failure",
            user_id=user_id,
            event_type="rate_limit_bypassed",
            error=str(exc),
        )
...
async def create_job(data, user: User, session: AsyncSession, idem: str | None) -> Job:
    ...
    job = Job(
        owner_id=user.id,
        status=JobStatus.queued if should_queue else JobStatus.pending,
    ...
    if should_queue:
        session.add(
            OutboxEvent(
                ...
                payload={"job_id": job.id, "user_id": user.id, "correlation_id": correlation_id},
            )
        )
    await session.commit()
    await session.refresh(job)
    await redisinfra.invalidate_pattern("jobs:*")
    await redisinfra.publish_status(
        job.id, job.status.value, user_id=user.id, correlation_id=correlation_id
    )
    log.info(
        "job_created",
        job_id=job.id,
...
def job_to_dict(job: Job) -> dict:
    return {field: getattr(job, field) for field in JOB_DICT_FIELDS}
...
async def list_jobs(user: User, session: AsyncSession, cursor: str | None, limit: int) -> dict:
    ...
    rows = list(
        (await session.scalars(stmt.order_by(desc(Job.created_at), desc(Job.id)).limit(limit + 1))).all()
    )
...
async def cancel_job(job_id: int, user: User, session: AsyncSession) -> Job:
    ...
    await redisinfra.invalidate_pattern("jobs:*")
    await redisinfra.publish_status(
        job.id, job.status.value, user_id=job.owner_id, correlation_id=job.correlation_id
    )
    return job
...
async def logs(job_id: int, user: User, session: AsyncSession) -> list[JobLog]:
    ...
    return (
        await session.scalars(
            select(JobLog).where(JobLog.job_id == job_id).order_by(JobLog.created_at, JobLog.id)
        )
    ).all()
