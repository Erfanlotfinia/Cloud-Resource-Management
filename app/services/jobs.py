...
async def rate_limit(user_id: int) -> None:
    ...
    try:
        ...
        log.warning("rate_limit_degraded", user_id=user_id, error=str(exc))


async def create_job(data, user: User, session: AsyncSession, idem: str | None) -> Job:
    await rate_limit(user.id)
    # Serialize per-user admission/idempotency checks. This becomes row-level locking on PostgreSQL
    # and is harmless in SQLite tests.
    await session.scalar(select(User.id).where(User.id == user.id).with_for_update())
    if idem:
        existing = await session.scalar(
            select(Job).where(Job.owner_id == user.id, Job.idempotency_key == idem)
        )
    ...
    reserved = await session.scalar(
        select(func.count())
        .select_from(Job)
        .where(Job.owner_id == user.id, Job.status.in_([JobStatus.queued, JobStatus.running]))
    )
    should_publish = (reserved or 0) < get_settings().jobs_per_user_running_limit
    job = Job(
        owner_id=user.id,
        status=JobStatus.queued if should_publish else JobStatus.pending,
    ...
    if should_publish:
        try:
            ...
            session.add(JobLog(job_id=job.id, message="job published"))
            await session.commit()
            await redisinfra.publish_status(job.id, job.status.value)
        except Exception as exc:
            ...
            session.add(
                JobLog(
                    ...
                    message=f"RabbitMQ publish failed: {exc}",
                )
            )
            await session.commit()
            await redisinfra.invalidate_pattern("jobs:*")
            raise AppError(
                ...
                "Job saved but could not be queued for execution",
                503,
            ) from exc
    return job
...
async def list_jobs(user: User, session: AsyncSession, cursor: str | None, limit: int) -> dict:
    ...
    if cached:
        log.info("jobs_cache_hit", key=key)
        return json.loads(cached)
    log.info("jobs_cache_miss", key=key)
    ...
    rows = list(
        (
            await session.scalars(
                stmt.order_by(desc(Job.created_at), desc(Job.id)).limit(limit + 1)
            )
        ).all()
    )
    ...
    items = rows[:limit]
    data = {
        "items": [job_to_dict(job) for job in items],
        "next_cursor": encode_cursor(items[-1].created_at, items[-1].id) if has_more and items else None,
        "has_more": has_more,
    }
    await redisinfra.safe_setex(
        key, get_settings().jobs_cache_ttl_seconds, json.dumps(data, default=str)
    )
    return data
...
async def logs(job_id: int, user: User, session: AsyncSession) -> list[JobLog]:
    ...
    return (
        await session.scalars(select(JobLog).where(JobLog.job_id == job_id).order_by(JobLog.created_at, JobLog.id))
    ).all()
