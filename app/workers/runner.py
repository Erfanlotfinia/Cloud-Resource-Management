...
async def execute(payload: dict) -> dict:
    ...
    if payload.get('non_retryable_fail'):
        raise NonRetryableExecutionError('non retryable job failure')
    if payload.get('should_fail'):
        raise RetryableExecutionError('simulated job failure')
    return {'ok': True, 'task_type': payload.get('task_type', 'demo')}


def lease_deadline():
    return datetime.now(timezone.utc) + timedelta(seconds=get_settings().job_lease_seconds)


def retry_delay(retry_count: int):
    ...
    async with SessionLocal() as session:
        job = await session.scalar(
            ...
            .where(
                Job.id == job_id,
                Job.status == JobStatus.queued,
                ((Job.locked_by.is_(None)) | (Job.lock_expires_at < now)),
            )
            .with_for_update(skip_locked=True)
        )
        if job is None:
        ...
        if (running or 0) >= get_settings().jobs_per_user_running_limit:
            ...
            await publish_status(
                job.id, 'pending', user_id=job.owner_id, correlation_id=job.correlation_id, worker_id=WORKER_ID
            )
            return None
        job.status = JobStatus.running
        job.locked_by = WORKER_ID
        job.lock_expires_at = lease_deadline()
        job.started_at = now
        session.add(JobLog(job_id=job.id, message=f'job lease acquired by {WORKER_ID}'))
        await session.commit()
        await invalidate_pattern('jobs:*')
        await publish_status(
            job.id, 'running', user_id=job.owner_id, correlation_id=job.correlation_id, worker_id=WORKER_ID
        )
        log.info(
            'job_claimed',
            job_id=job.id,
...
async def process(job_id: int) -> None:
    ...
    stop = asyncio.Event()
    renew_task = asyncio.create_task(renew_lease(job_id, stop))
    try:
        ...
        async with SessionLocal() as session:
            ...
            await invalidate_pattern('jobs:*')
            await publish_status(
                job.id, 'completed', user_id=owner_id, correlation_id=correlation_id, worker_id=WORKER_ID
            )
            log.info(
                'job_completed',
                job_id=job.id,
...
async def fail_job(job_id: int, owner_id: int, correlation_id: str, exc: Exception, retryable: bool):
    async with SessionLocal() as session:
        job = await session.scalar(
            select(Job).where(Job.id == job_id, Job.locked_by == WORKER_ID).with_for_update()
        )
        ...
        if retryable and job.retry_count <= job.max_retries:
            ...
            session.add(
                JobLog(
                    ...
                    message=f'retry scheduled {job.retry_count} after {retry_delay(job.retry_count)}s',
                )
            )
            event = OutboxEvent(
                event_type='job.retry_queued',
                payload={'job_id': job.id, 'user_id': owner_id, 'correlation_id': correlation_id},
            )
            event.next_attempt_at = datetime.now(timezone.utc) + timedelta(
                seconds=retry_delay(job.retry_count)
            )
            session.add(event)
            status = 'queued'
        else:
        ...
        await invalidate_pattern('jobs:*')
        await publish_status(
            job.id, status, user_id=owner_id, correlation_id=correlation_id, worker_id=WORKER_ID
        )
        log.warning(
    ...
    async with SessionLocal() as session:
        jobs = list(
            (
                await session.scalars(
                    select(Job)
                    .where(Job.status == JobStatus.running, Job.lock_expires_at < now)
                    .with_for_update(skip_locked=True)
                    .limit(100)
                )
            ).all()
        ...
        for job in jobs:
            ...
            if job.status == JobStatus.queued:
                await enqueue_outbox(session, job, 'job.lease_recovered')
            else:
...
async def recovery_loop():
    while True:
        await recover_expired_leases_once()
        await asyncio.sleep(max(5, get_settings().job_lease_seconds // 2))
...
async def main() -> None:
    ...
    queue = await channel.declare_queue(
        get_settings().jobs_queue_name, durable=True, arguments=jobs_queue_arguments()
    )
