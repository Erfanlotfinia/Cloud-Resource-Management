...
async def create(data:JobCreate, idempotency_key:str|None=Header(default=None, alias='Idempotency-Key'), user:User=Depends(current_user), session:AsyncSession=Depends(get_session)): return await svc.create_job(data,user,session,idempotency_key)
@router.get('', response_model=JobListResponse)
async def list_(cursor:str|None=None, limit:int=20, user:User=Depends(current_user), session:AsyncSession=Depends(get_session)): return await svc.list_jobs(user,session,cursor,limit)
@router.get('/{job_id}', response_model=JobResponse)
async def get(job_id:int,user:User=Depends(current_user),session:AsyncSession=Depends(get_session)): return await svc.get_job(job_id,user,session)
@router.post('/{job_id}/cancel', response_model=JobResponse)
async def cancel(job_id:int,user:User=Depends(current_user),session:AsyncSession=Depends(get_session)): return await svc.cancel_job(job_id,user,session)
