...
async def safe_get(key: str):
    try:
        ...
        log.warning('redis_cache_fallback_to_postgres', event_type='redis_cache_failure', error=str(exc))
        return None
...
async def publish_status(
    ...
    try:
        ...
        log.warning(
            'sse_realtime_push_disabled_due_to_redis_failure',
            job_id=job_id,
            user_id=user_id,
            event_type='sse_degraded',
            correlation_id=correlation_id,
            worker_id=worker_id,
            error=str(exc),
        )
